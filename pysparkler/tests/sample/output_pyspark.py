...
tz_df = spark.createDataFrame([28801], "long").selectExpr("timestamp(value) as ts")  # PY40-41-005: As of PySpark 4.1, spark.sql.execution.pandas.convertToArrowArraySafely is enabled by default, so PyArrow raises errors on unsafe conversions (integer overflow, float truncation, loss of precision) in Arrow-enabled UDFs and when creating DataFrames from pandas. To restore the previous behavior, set it to false.  # noqa: E501
tz_df.show()
...
rp_df = spark.createDataFrame([
        ...
        (None, None, "Tom", None),
        (None, None, None, True)],
        schema=["age", "height", "name", "bool"])  # PY40-41-005: As of PySpark 4.1, spark.sql.execution.pandas.convertToArrowArraySafely is enabled by default, so PyArrow raises errors on unsafe conversions (integer overflow, float truncation, loss of precision) in Arrow-enabled UDFs and when creating DataFrames from pandas. To restore the previous behavior, set it to false.  # noqa: E501
